...
loads. It is written for an AI operator (Codex or Claude Code), not a human onboarding
guide. The parallel file [`CLAUDE.md`](CLAUDE.md) is the same contract for Claude Code
hosts (tool/host references swapped). Keep the two in sync if you edit either.
...
(e.g. `codex.exe --print --model sonnet`) rather than a raw bearer token, so the host
...
```
Blacksite/
  AGENTS.md / AGENTS.md      ← this engine prompt (CC / Codex variants)
...
Constitutional rule for ALL written artifacts: AGENTS.md/AGENTS.md, INSTANCE.md, SOPs,
